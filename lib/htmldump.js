...
function dumpArticle (options, title, oldid, contentHost) {
    var checkRevision;
    if (options.store) {
    ...
    .then(function(checkResult) {
        if (!checkResult) {
            ...
            var url = contentHost + '/page/html/'
		+ encodeURIComponent(title) + '/' + oldid
		+ '?redirect=false';
            return preq.get({
                uri: url,
                headers: {
...
Dumper.prototype.next = function () {
    ...
    .then(function(article) {
        ...
        var title = article[0];
        var oldid = article[1];
        return dumpArticle(self.options, title, oldid, self.contentHost)
        .catch(function(e) {
